...
1. ``RuntimeCG(string& shmName, const vector<int>& shapes, string& enginePath, bool ultralytics)``;
   ...
   * ultralytics: 读取文件格式 默认false
     ...
     ```
2. ``RuntimeCG(void* image_ptr, const vector<int>& shapes, string& enginePath, bool ultralytics)``;
...
* 原有TensorRT的输出格式
* 怎么获取输出 对实例化进行Numpy ``outputPtr = np.array(detect, copy=False) #存储结果的buffer``
* Numpy一次后永久有效可重复从output获取结果，不需要重复np.array(detect, copy=False)(除非更换engine）

  ```
  outputPtr = np.array(detect, copy=False) #存储结果的buffer
  detect.predict()
  detect.predict()
  ```
* 更换engine后需重新对实例化进行Numpy
  ...
  ```
  outputPtr = np.array(shot, copy=False)
  detect.setEnginePath("xxx.engine", True)
  outputPtr = np.array(shot, copy=False)
  ```
